...
install_atatus_ext() {
    ...
    if [[ -n "$ATATUS_LICENSE_KEY" ]] && ! php -n $(which composer2) show -d "$build_dir/.heroku/php" --installed --quiet heroku-sys/ext-atatus 2>/dev/null; then
        if php -n $(which composer2) require --update-no-dev -d "$build_dir/.heroku/php" -- "heroku-sys/ext-atatus:*" >> $build_dir/.heroku/php/install.log 2>&1; then
